...
switch($_SERVER['REQUEST_METHOD']) :
    case 'GET' :
        ...
        $Qstring = file_get_contents('php://input');
        if($Qstring) :
            $sql .= " WHERE ";
            $Qstring = json_decode($Qstring, true);
            //print_r($Qstring);
            foreach($Qstring['fields'] AS $field => $value):
                $sql .= " $field = '$value' ".$Qstring['operator'];
            endforeach;
            $sql = rtrim($sql,$Qstring['operator']);
        endif;
